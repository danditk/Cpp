...
double secDay = 86400; // stala ilosc sekund w dniu
double czasObzartucha, ciastka, pudelka; // deklaracja tablic o dlugosci zaleznej od pobranych zmiennych
...
int main()
{
    ...
    for(int i=0; i<testQty; i++){ // petla powtarzajaca sie w zaleznosci od ilosci testow, jakie maja byc przeprowadzone
        ...
        for(int j=0; j<mObzatruchy; j++){ // petla uwzgledniajaca wszytkich obzartuchow
            ...
            ciastka += floor(secDay/czasObzartucha); // obliczanie ilosci ciastek jakie podczas doby zje jeden obzartuch, sumowanie kolejnych ciastek zjedzonych przez obzartucha
        }
        ...
        pudelka = ceil(ciastka/nCiastkaPudelko);
        cout << pudelka << endl; // nie przyjmowa³ SPOJ, bo nie by³o tutaj end lajna :D
        ciastka=0; pudelka=0;
    }
    return 0;
}
